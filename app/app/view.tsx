...
import { ticketStatusMeta } from "@/lib/mock-data"
import type { Client, Ticket } from "@/lib/mock-data"

export function ClientPortalView({ client, myTickets }: { client: Client; myTickets: Ticket[] }) {
  const [, setTab] = useState<"resumen" | "actividad">("resumen")
  void setTab
    ...
    <div className="flex flex-col gap-5 px-4 pb-6 pt-2">
      ...
      <section>
        ...
        <div className="flex flex-col gap-2">
          {myTickets.map((t) => {
            const meta = ticketStatusMeta[t.status] ?? { label: t.status, class: "" }
            return (
          ...
          {myTickets.length === 0 && (
            <Card className="border-border/70 p-4 text-center text-sm text-muted-foreground">
