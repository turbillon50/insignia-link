...
import type { Client, Ticket, ServiceVisit } from "@/lib/mock-data"
...
export function TecnicoRutaView({
  routeSummary,
  current,
  currentClient,
  currentTicket,
  ...
  routeSummary: { services: number; completed: number; pending: number }
  current: ServiceVisit | null
  currentClient: Client | null
  currentTicket: Ticket | null
  next: ServiceVisit | null
  nextClient: Client | null
}) {
  const [started, setStarted] = useState(false)
  const prio = currentTicket?.priority ?? "media"
  const prioMeta = priorityMeta[prio] ?? { label: prio, class: "" }
      ...
      {current && (
        <Card className="border-primary/30 bg-primary/[0.04] p-4">
          <div className="flex items-center justify-between">
            ...
            <StatusPill tone={prio} label={prioMeta.label} />
          </div>
          ...
          <Button className="mt-4 h-11 w-full" variant={started ? "outline" : "default"} onClick={() => setStarted((s) => !s)}>
            {started ? <>Servicio en progreso…</> : <><Play className="h-4 w-4" /> Iniciar servicio</>}
          </Button>
        </Card>
      )}
